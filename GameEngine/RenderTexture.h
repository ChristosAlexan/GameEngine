...
public:
	...
	bool RenderCubeMapFace(ID3D11Device* device, ID3D11DeviceContext* deviceContext, int index, unsigned int& mipLevel, ID3D11DepthStencilView* depthView, float* rgba, bool copyDepth, bool bSave = false);
	void RenderAllCubeFaces(ID3D11Device* device, ID3D11DeviceContext* deviceContext, ID3D11DepthStencilView* depthView, float* rgba, bool copyDepth);
...
public:
	...
	Microsoft::WRL::ComPtr <ID3D11RenderTargetView> m_renderTargetViews[6];
