...
float4 main(PS_INPUT input) : SV_TARGET
{
    ...
    int2 sampleIndices = int2(input.uv * float2(textureSize.x, textureSize.y));
    float4 worldPos = worldPositionTexture.Load(int3(sampleIndices, 0));
    float3 camPos = cameraPos.xyz;
    float distance = length(worldPos.xyz - camPos);
    
    float minDist = 0.0f;
    float maxDist = 50.0f;

    float softness = saturate((distance - minDist) / (maxDist - minDist)); // 0 -> 1
    softness = lerp(1.0f, 0.1f, softness); // now: 1.0 when close, 0.1 when far
